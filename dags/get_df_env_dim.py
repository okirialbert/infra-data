"""Get (DSD_GG@DF_ENV_DIM) Green Growth - Environmental dimension of quality of life dataset from OECD API"""
...
task_get_data_oecd = SimpleHttpOperator(
    ...
    endpoint="OECD.ENV.EPI/DSD_GG@DF_ENV_DIM/1.0/*.A.*.*.*?c[TIME_PERIOD]=ge:1981+le:2021&attributes=dsd&measures=all&updatedAfter=2015-01-01T00:00:00.000-01:00",
...
task_get_data_structure_oecd = SimpleHttpOperator(
    ...
    endpoint="OECD.ENV.EPI/DSD_GG@DF_ENV_DIM/1.0?references=all",
...
def read_store(**kwargs):
    ...
    with open(csv_path, 'r') as a:
        curr.copy_from(a, 'df_env_dim', sep=',', null="")
        conn.commit()
